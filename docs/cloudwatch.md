...
- **W3C Distributed Trace Correlation:** Setting `enableXRay: true`, including `"http"` in `telemetries`, and enabling `addXRayTraceIdHeader: true` (or specifying allowed API target domains) injects standard `X-Amzn-Trace-Id` headers into client HTTP requests. This links client web sessions directly to backend AWS X-Ray server traces across the Application Load Balancer and application services.
...
- **Application Signals Billing Modes:**
  1. **Transaction Search Mode (Golden Signals + Trace Spans):** Billed per Golden Signal ($1.50 per 1M signals) AND per GB trace span ingested ($0.35 per GB).
  2. **Golden-Metrics-Only Mode (Trace Ingestion Disabled):** Billed strictly per Golden Signal ($1.50 per 1M signals), with trace span ingestion disabled.
...
- **Baseline RUM Workload:** 250,000 monthly sessions × 20 events/session = 5,000,000 events = **$50.00 USD/month steady-state (~RM 225.00 MYR)** *(Note: Initial trial period cost with 1M-event free trial is $40.00 USD / RM 180.00 MYR)*.
- **Moderate RUM Workload:** 1,000,000 monthly sessions × 20 events/session = 20,000,000 events = **$200.00 USD/month steady-state (~RM 900.00 MYR)** *(Note: Initial trial period cost with 1M-event free trial is $190.00 USD / RM 855.00 MYR)*.
...
Below is the cost scaling comparison across throughput levels under **Transaction Search Mode** (Golden Signals @ $1.50/1M + Trace Span Ingestion @ $0.35/GB assuming ~2 downstream spans per transaction @ ~1.8 KB/span) versus **Golden-Metrics-Only Mode** (Trace Span Ingestion disabled):

| Sustained Load | Signals / month | Trace Ingestion | Transaction Search Cost (USD) | Golden-Metrics-Only Cost (USD) |
| --- | --- | --- | --- | --- |
| **1,000 req/min** | 131,000,000 | ~246 GB | **$259.79** | **$196.50** |
| **5,000 req/min** | 657,000,000 | ~1.23 TB | **$998.92** | **$702.75** |
| **10,000 req/min** | 1,310,000,000 | ~2.46 TB | **$1,781.51** | **$1,194.00** |
| **25,000 req/min** | 3,290,000,000 | ~6.16 TB | **$3,666.29** | **$2,385.00** |

*Note: At ~10,000 req/min sustained under Transaction Search Mode, un-sampled APM span ingestion ($1,781.51 USD / RM 8,017 MYR) exceeds fixed host-unit Dynatrace licensing ($870–$1,110 USD). To prevent cost escalation:*
1. **Apply Intelligent Trace Sampling:** In Transaction Search Mode, maintain 5% steady-state trace sampling, ramping up to 100% capture strictly on 5xx errors and latency anomalies.
2. **Switch to Golden-Metrics-Only Mode for Payment Gateway:** Disable span ingestion for the payment service path to cap costs strictly at Golden Signals ($1.50/1M signals).
3. **Audit Pipeline Separation:** Route compliance audit trails to CloudWatch Logs or S3, avoiding APM span ingestion for compliance retention.
4. **Leverage Free Trial:** Utilize the 3-month CloudWatch Application Signals free trial:
   - When **Transaction Search Mode** is enabled: 3 months up to **100 GB of data ingestion** or **1 million spans indexed as X-Ray trace summaries** (whichever comes first).
