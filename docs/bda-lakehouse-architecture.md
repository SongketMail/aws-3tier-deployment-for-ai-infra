...
* **Departmental Domains:**
  * **Department of Wildlife Conservation (DWC):** Handles wildlife encounters, species monitoring, and spatial movement corridors.
  * **Department of Mineral & Geoscience (DMG):** Maps hydrogeological borehole reserves and calculates landslide hazard stability.
  * **Department of Forestry (DOF):** Tracks forest fire incidents, biomass susceptibility, and concession boundaries.
  * **National Water & Climate Institute (NWCI):** Models hydrological vulnerability and climate adaptation indices.
  * **Meteorological Services Department (MSD):** Serves precipitation curves and weather telemetry feeds.
  * **Global Satellite Thermal Sensing Network (GSTRN):** Streams thermal anomaly and hotspot telemetry.

---

## Architectural Deconstruction of the Legacy BDA Environment

The legacy Big Data Analytics (BDA) platform operated as a early-generation big data deployment characterized by tightly coupled compute and storage architectures, unmanaged database proliferation, fragile point-to-point data ingestion methods, and proprietary visualization systems.
...
1. **Persistent Object Storage & WORM Immutability:** Uses Ceph (via RADOS Gateway) or MinIO Enterprise Object Store. Configuring object buckets with S3 Object Lock in Compliance Mode establishes hardware-grade Write-Once-Read-Many (WORM) immutability, ensuring master records cannot be overwritten or prematurely purged.
...
| :--- | :--- | :--- | :--- |
| **Distributed Object Storage** | Hadoop HDFS, GlusterFS. | Ceph Object Storage / MinIO Object Store. | Horizontal scale-out; unified S3 API; hardware-grade WORM S3 Object Lock immutability; small-file bottleneck elimination. |
...
* **Security Boundaries:** MCP servers run in isolated containers within a dedicated DMZ. Database connections operate with `SET SESSION CHARACTERISTICS AS TRANSACTION READ ONLY`. Outputs are written strictly to Tier 2 scratch storage and tagged with `ai_generated_data: true`.
...
* **Visualization Overhaul (Apache Superset + deck.gl):** Replaces proprietary Tableau Server with Apache Superset. Connects to Trino via SQLAlchemy, leveraging deck.gl for hardware-accelerated 3D geospatial rendering and Keycloak Row-Level Security (RLS).
...
1. **Persistent Cloud Storage (Tier 0, 1, 2):**
   * **Tier 0 SSoT:** Amazon S3 Buckets configured with **S3 Object Lock in Compliance Mode** (WORM), ensuring hardware-grade immutability for golden datasets.
...
2. **Lakehouse Catalog & Metadata:**
   * **AWS Glue Data Catalog** or **Apache Polaris REST Catalog** hosted on Amazon EKS, managing Apache Iceberg table commits and Parquet file manifests.
...
4. **Ingestion & Pipeline Orchestration:**
   * **Amazon Managed Workflows for Apache Airflow (MWAA):** Orchestrates batch pipelines, ODCS data contract CLI validation, and OpenLineage events.
...
3. **Hybrid Secure Connectivity & MCP Integration:**
   * **AWS Direct Connect / Dedicated IPsec VPN:** Low-latency 10GbE / 1GbE encrypted tunnel connecting the on-premises GPU cluster directly to the AWS VPC.
   ...
   * **Data Quarantine Enforcement:** All AI inferencing occurs on-premises. Model intermediate outputs are stored on local S3-compatible storage (MinIO) marked as Tier 2 Scratch. AI models have zero write permissions back to Cloud Tier 0 SSoT.
...
|                 v                                                       v                     |
|  +----------------------------------------+           +------------------------------------+  |
|  | RKE2 MAIN CLUSTER (14 NODES)           |           | K3S SUPPORTING CLUSTER (5 NODES)   |  |
...
* **Proxmox VE Cluster:** High-density enterprise hypervisor cluster with 100GbE data plane and 10GbE out-of-band management plane.
* **PCIe GPU Passthrough:** Direct hardware assignment of NVIDIA H100/A100/L40S GPUs to virtualized AI worker nodes, bypassing hypervisor overhead for maximum matrix math performance.

#### 2. Dual-Cluster Open-Source Kubernetes Architecture (RKE2 & K3s)
* **Cluster A: Main Production Cluster (14 Nodes - RKE2):**
  * Built on Rancher Kubernetes Engine 2 (RKE2 v1.30 LTS) with Cilium eBPF CNI, FIPS compliance readiness, containerd runtime, and SELinux/AppArmor enforcement.
  * **3x Control Plane / Server Nodes:** High-Availability etcd quorum (`rke2-cp-01` to `03`).
  * **4x AI / GPU Worker Nodes:** Executes Ollama, vLLM, RAGFlow, and Spark/Sedona distributed spatial workloads (`rke2-worker-ai-01` to `04`).
  * **4x Application / Microservices Worker Nodes:** Hosts Apache NiFi, Airflow, APISIX, Keycloak, Superset, Next.js, and Trino engines (`rke2-worker-app-01` to `04`).
  * **3x Database & Stateful Worker Nodes:** Hosts PostgreSQL Patroni nodes and local Ceph OSD storage daemons (`rke2-worker-db-01` to `03`).
* **Cluster B: Supporting Services Cluster (5 Nodes - K3s):**
  * Built on lightweight K3s (v1.30 LTS) with embedded etcd.
  * **3x Control Plane Nodes** (`k3s-mgmt-01` to `03`).
  * **2x Dedicated Worker Agent Nodes** (`k3s-worker-01` to `02`) hosting Prometheus, Grafana, Loki, HashiCorp Vault, and local CI/CD runners.
...
* **RADOS Gateway (S3 Object Storage):** Supplies S3-compatible APIs. Buckets configured with **S3 Object Lock in Compliance Mode** provide hardware-grade WORM storage for Tier 0 Golden SSoT records.
* **Storage Provisioning Types:**
  ...
  * **Local Path Provisioner:** Ultra-low latency node-local NVMe storage for ephemeral Tier 2 AI scratchpads.
...
└── Formalize ODCS v3.1.0 data contracts across all five business domains
...
└── Validate 30-day parallel operational run across all five business cases
...
| **Phase 4: Cutover & Decommissioning (Months 10–12)** | Tableau Server cluster, legacy CMS, custom application portals. | Apache Superset (deck.gl), Next.js / React portal, Apache APISIX. | Discrepancies between Tableau and Superset spatial maps; user resistance to new UI. | Run 30-day side-by-side verification runs; validate geospatial rendering against PostGIS base layers; conduct agency training. |
...
2. **Absolute Data Integrity & Immutability:** Apache Iceberg delivers serialized ACID transactions, snapshot time-travel, and schema evolution. S3 Object Lock in Compliance Mode guarantees physical WORM immutability for Tier 0 golden records.
