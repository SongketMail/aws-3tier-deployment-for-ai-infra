...
1. **Persistent Object Storage & Immutability Contract:** Uses Ceph (via RADOS Gateway), MinIO Enterprise Object Store, or AWS S3.
   ...
   * **Iceberg Maintenance Separation:** Immutable source/evidence storage is separated from mutable analytical lakehouse tables. Apache Iceberg tables reside in dedicated buckets or partitions configured with **Governance Mode** Object Lock or unlocked object paths. Catalog maintenance jobs (such as snapshot compaction, orphan file purging, and snapshot expiration) operate either on unlocked object paths or via a narrowly scoped maintenance service role authorized with `s3:BypassGovernanceRetention` permission sending the `x-amz-bypass-governance-retention: true` request header. Retention compatibility is verified via automated validation tests prior to production compaction execution.
...
*Portability Qualification:* Solution 1 leverages open-source data specifications (Apache Iceberg tables and Apache Parquet files) alongside selected open-source applications (Keycloak, Apache APISIX, OpenMetadata, Apache Superset), but replaces self-hosted platform components with AWS-managed cloud services (AWS Glue Data Catalog, AWS MWAA Airflow, Amazon Bedrock, Amazon Athena, and Amazon EMR Serverless). Consequently, Solution 1 is cloud-dependent and AWS-managed rather than fully vendor-neutral. Organizations requiring complete vendor neutrality and portability should deploy Solution 3 (100% On-Premises) or self-host all components on Amazon EKS.
...
3. **Hybrid Secure Connectivity & MCP Integration:**
   * **AWS Direct Connect / Dedicated IPsec VPN:** AWS Direct Connect provides a dedicated private physical circuit (unencrypted by default; Layer 2 MACsec encryption is available for dedicated 10 Gbps, 100 Gbps, and 400 Gbps connections at selected Direct Connect locations, whereas 1 Gbps connections or non-MACsec locations require a Layer 3 IPsec VPN overlay when encryption in transit is required) or a dedicated IPsec VPN tunnel connecting the on-premises GPU cluster directly to the AWS VPC.
...
* **Proxmox VE Cluster Sizing & Host Topology:** High-density enterprise hypervisor cluster comprising **11 physical Proxmox VE hosts** (4x AI/GPU hosts, 4x Application hosts, 3x Database/Stateful hosts) with a 100GbE data plane and 10GbE out-of-band management plane. Minimum per-host hardware capacity requirements:
  * **4x AI/GPU Compute Hosts:** Dual 32-core CPUs (e.g. AMD EPYC / Intel Xeon), 512GB RAM, 8TB NVMe storage, and 2x NVIDIA H100/A100 GPUs per physical host.
...
* **VM Sizing, Anti-Affinity & N+1 Headroom:** The 14 virtualized RKE2 production nodes and 5 virtualized K3s management nodes execute as Virtual Machines (VMs) provisioned across the 11 physical PVE hosts. Strict Proxmox VE VM anti-affinity rules are enforced across physical hosts to prevent single points of failure (SPOFs) (e.g. no two RKE2/K3s control-plane VMs or Patroni DB VMs reside on the same physical PVE host). Each host tier maintains an N+1 capacity reservation (reserving 15–20% CPU/RAM headroom per tier) so that upon any single physical host failure, affected virtualized cluster VMs automatically restart on remaining physical hosts, guaranteeing high availability.
