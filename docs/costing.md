...
| **Database Tier (RDS)** | Multi-AZ `db.m6g.large` PostgreSQL **[TS-06]**<br><br>• 2 vCPU, 8GB RAM (High Availability)<br><br>• 50GB gp3 Storage | $0.304 / hr<br><br>$0.23 / GB-month | $221.92<br><br>$11.50 |
| **Caching Tier** | **Amazon ElastiCache Valkey (`cache.t4g.micro`) [TS-06, TS-05]**<br><br>• 1 Node, 0.5 GB RAM (Session & metadata caching)<br>• *Valkey pricing is 20% lower than legacy Redis OSS* | $0.0128 / hr | $9.34 |
...
| **Database Tier (RDS)** | **Multi-AZ `db.m6g.xlarge` PostgreSQL [TS-06]**<br><br>• 4 vCPU, 16GB RAM (Matches Server 04 Data Tier)<br><br>• 50GB gp3 Multi-AZ Storage | $0.608 / hr<br><br>$0.46 / GB-month | $443.84<br><br>$23.00 |
| **Caching Tier** | **Amazon ElastiCache Valkey (`cache.t4g.medium`) [TS-06, TS-05]**<br><br>• 1 Node, 3.09 GB RAM (Production cache & task broker)<br>• *Valkey pricing is 20% lower than legacy Redis OSS* | $0.0544 / hr | $39.71 |
| **Storage Tier (S3)** | Amazon S3 (Encrypted Uploads & Media)<br><br>• ~100 GB Standard Storage<br><br>• ~50,000 PUT/GET API requests | $0.023 / GB-month<br><br>Nominal rates | $2.30<br><br>$0.50 |
...
| AWS Service / Line Item | 12-Month Cumulative Spend (USD) | Local Currency Equivalent (MYR) | Percentage of Total Spend |
| :--- | :--- | :--- | :--- |
| **Amazon ElastiCache (Valkey / Redis)** | $13,991.93 | RM 62,963.70 | 22.79% |
| **Amazon EC2 (Compute Instances)** | $13,315.42 | RM 59,919.39 | 21.69% |
| **Amazon Elastic File System (EFS)** | $10,807.09 | RM 48,631.91 | 17.60% |
| **Amazon Relational Database Service (RDS)** | $10,295.35 | RM 46,329.06 | 16.77% |
| **Amazon EC2-Other (EBS, NAT Gateway, Data Transfer)** | $7,009.31 | RM 31,541.88 | 11.42% |
| **Elastic Load Balancing (ALB)** | $3,555.55 | RM 15,999.96 | 5.79% |
| **Amazon VPC (Public IPv4 & Flow Logs)** | $917.73 | RM 4,129.77 | 1.49% |
| **Amazon CloudWatch** | $890.24 | RM 4,006.09 | 1.45% |
| **AWS WAF (Web Application Firewall)** | $734.44 | RM 3,304.98 | 1.20% |
| **AWS Backup** | $56.42 | RM 253.88 | 0.09% |
| **AWS Secrets Manager** | $34.79 | RM 156.56 | 0.06% |
| **Amazon S3** | $15.93 | RM 71.71 | 0.03% |
| **Amazon Route 53** | $11.08 | RM 49.86 | 0.02% |
| **AWS Key Management Service (KMS)** | $11.08 | RM 49.85 | 0.02% |
| **AWS Cost Explorer** | $4.09 | RM 18.41 | 0.01% |
| **Data Transfer Adjustments / Credits** | -$249.98 | -RM 1,124.92 | -0.41% |
| **GRAND TOTAL (12-MONTH PERIOD)** | **$61,400.47 USD** | **RM 276,302.12 MYR** | **100.00%** |

#### B. Monthly Expenditure Progression (Sept 2025 – Aug 2026)
The monthly spending curve illustrates the operational progression from initial sandbox setup to high-load scaling:

| Billing Month | Monthly Spend (USD) | Monthly Spend (MYR) | Key Architectural Milestones |
| :--- | :--- | :--- | :--- |
| **2025-09-01** | $2,805.25 | RM 12,623.64 | Initial VPC deployment, single-AZ RDS & Valkey provisioning |
| **2025-10-01** | $4,029.27 | RM 18,131.72 | Addition of EFS shared storage and ASG compute nodes |
| **2025-11-01** | $4,433.64 | RM 19,951.38 | Multi-AZ database setup and WAF rules integration |
| **2025-12-01** | $4,802.65 | RM 21,611.95 | Staging environment pre-load testing and vector index baking |
| **2026-01-01** | $8,940.71 | RM 40,233.18 | Peak load-test execution & high-memory caching expansion (`cache.r6g.2xlarge`) |
| **2026-02-01** | $6,726.76 | RM 30,270.42 | Post-load-test stabilization and capacity tuning |
| **2026-03-01** | $6,483.23 | RM 29,174.52 | Production preparation & Langfuse observability pipeline active |
| **2026-04-01** | $5,988.17 | RM 26,946.75 | Steady-state production run-rate with t4g/c8g instance mix |
| **2026-05-01** | $6,015.75 | RM 27,070.86 | Continuous production operation and EFS model weight caching |
| **2026-06-01** | $6,113.60 | RM 27,511.18 | Full-scale enterprise AI workload execution (RAGFlow + pgvector) |
| **2026-07-01** | $527.63 | RM 2,374.35 | Mid-year optimization, maintenance, and credit adjustment cycle |
| **2026-08-01** | $4,533.82 | RM 20,402.18 | Optimized post-audit production baseline |

#### C. August 2026 Instance Type Distribution
An analysis of instance-type cost allocations for August 2026 (**$6,505.72 USD** gross compute & serverless total) demonstrates the exact hardware distribution in `ap-southeast-5`:

| Instance Type / Component | Allocation Category | Monthly Cost (USD) | Monthly Cost (MYR) |
| :--- | :--- | :--- | :--- |
| **No instance type (Serverless, S3, ALB, EBS, NAT)** | Managed Services | $2,502.50 | RM 11,261.25 |
| **`cache.r6g.2xlarge`** | High-Memory ElastiCache Valkey | $1,494.55 | RM 6,725.46 |
| **`c8g.large`** | Graviton4 Compute (App & RAG Workers) | $1,009.54 | RM 4,542.95 |
| **`db.m7g.xlarge`** | Graviton3 RDS PostgreSQL (Multi-AZ Primary) | $592.22 | RM 2,665.01 |
| **`db.m7g.large`** | Graviton3 RDS PostgreSQL (Read Replica) | $296.11 | RM 1,332.50 |
| **`c6g.2xlarge`** | Graviton2 Compute (High-Throughput Processing) | $198.11 | RM 891.48 |
| **`c8g.xlarge`** | Graviton4 Compute (AI Vector Indexing) | $178.98 | RM 805.43 |
| **`cache.t4g.medium`** | Graviton2 ElastiCache Valkey (Session Caching) | $51.19 | RM 230.34 |
| **`c6g.large`** | Graviton2 Compute (Backend API Services) | $49.51 | RM 222.78 |
| **`c6g.medium`** | Graviton2 Compute (Microservices) | $49.45 | RM 222.54 |
| **`t4g.medium`** | Graviton2 Burstable Compute (Utility Workers) | $28.42 | RM 127.89 |
| **`t3.micro`** | x86 Burstable Compute (Utility Services) | $26.56 | RM 119.52 |
| **`t3.small`** | x86 Burstable Compute (Staging Utility) | $17.71 | RM 79.68 |
| **`t2.nano`** | x86 Legacy Micro Nodes | $10.86 | RM 48.88 |
| **TOTAL (AUGUST 2026 INSTANCE DISTRIBUTION)** | | **$6,505.72 USD** | **RM 29,275.74 MYR** |

#### D. June 2026 Daily Run-Rate Audit ($19,174.98 USD June Aggregate Telemetry)
Granular daily tracking across June 2026 shows extreme operational consistency across core infrastructure components:
- **EC2 Compute Instances:** $50.55 – $50.77 / day (Average ~$50.68 / day)
- **ElastiCache Valkey:** $49.90 / day (Fixed multi-node cluster pricing)
- **RDS PostgreSQL:** $32.08 / day (Fixed Multi-AZ instance charge)
- **EC2-Other (EBS, NAT Gateway, Data Transfer):** $22.28 – $24.28 / day
- **Elastic Load Balancing (ALB):** $7.95 – $15.82 / day (Varies dynamically with traffic LCU surges)
...
* **Native Engine Telemetry ($0.00 / Free Tier):**
  * **RDS PostgreSQL:** `CPUUtilization`, `FreeableMemory`, `FreeStorageSpace`, `ReadIOPS`, `WriteIOPS` (Standard 1-min/5-min metrics free; OS Enhanced Monitoring logs billed via CloudWatch Logs ingestion).
...
* **EC2 Guest OS Telemetry (Unified CloudWatch Agent):**
  * Emits custom memory (`mem_used_percent`, `mem_available`), disk storage (`disk_used_percent`, `disk_free`), and network interface (`bytes_sent`, `bytes_recv`, `drop_in`, `drop_out`) metrics via `amazon-cloudwatch-agent`.
  * Cost footprint: 8 custom metrics per node @ $0.30/metric/month. For a 15-instance cluster (120 custom metrics total, 110 billable after 10 free metrics), this establishes an incremental run-rate of **$33.00 USD/month** (~**RM 148.50 MYR**).
* **Total Observability Envelope:** Integrating CloudWatch RUM ($50.00–$200.00 USD) and CloudWatch Host Agent metrics ($33.00 USD) delivers full front-to-back operational visibility for under **$238.00 USD/month** (~**RM 1,071.00 MYR**), deprecating third-party agent licensing within AWS.
...
| :--- | :--- | :--- | :--- | :--- |
| **Amazon Bedrock [MC-01, MC-02]** *(Alternative to OpenAI API)* | • **Claude 3.5 Sonnet / Qwen3** (Chat & Reasoning)<br> - 1 Million Input Tokens / mo<br> - 500,000 Output Tokens / mo<br><br>• **Qwen3/Cohere Embed Multilingual**<br> - 5 Million embedding tokens / mo | Input: $0.003 / 1k tokens<br>Output: $0.015 / 1k tokens<br><br>Embed: $0.0001 / 1k tokens | **$11.00**<br><br>• Claude Input: $3.00<br>• Claude Output: $7.50<br>• Cohere Embed: $0.50 | **~RM 49.50** |
| **Amazon Cognito User Pools [TS-05]** *(Alternative to Auth0 / Self-Hosted Auth)* | • **15,000 Monthly Active Users (MAUs)**<br><br>• First 10,000 MAUs: **Free**<br><br>• Standard User Pools active traffic | $0.00 (First 10k MAUs)<br><br>$0.0055 / MAU (Next 5k MAUs) | **$27.50** | **~RM 123.75** |
...
By replacing high-cost vendor-managed SIEM platforms with our self-hosted, standalone Wazuh SIEM deployment (Option B under [`docs/wazuh.md`](wazuh.html)), we achieve significant savings:
